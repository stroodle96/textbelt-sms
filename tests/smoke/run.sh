...
if [[ "${LIVE_SMOKE:-0}" != 1 ]]; then
  python3 -c 'import json,sys,urllib.request; data=json.load(urllib.request.urlopen(sys.argv[1] + "/requests"))["requests"]; assert len(data) == 1, data; assert data[0] == {"phone":"+15551234567","message":"smoke","key":"smoke-test-key"}, data' "$stub"
  curl --fail --silent -X POST "$stub/mode/failure" >/dev/null
  python3 "$smoke_dir/exercise_api.py" --token "$token" --failure
  python3 -c 'import json,sys,urllib.request; data=json.load(urllib.request.urlopen(sys.argv[1] + "/requests"))["requests"]; assert len(data) == 2, data; assert data[1] == {"phone":"+15551234567","message":"smoke","key":"smoke-test-key"}, data' "$stub"
...
if [[ "${LIVE_SMOKE:-0}" != 1 ]]; then
  python3 -c 'import json,sys,urllib.request; data=json.load(urllib.request.urlopen(sys.argv[1] + "/requests"))["requests"]; assert len(data) == 3, data; assert data[2] == {"phone":"+15551234567","message":"restart-smoke","key":"smoke-test-key"}, data' "$stub"
fi
